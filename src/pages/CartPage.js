...
const CartPage = () => {
    ...
    return (
        <main className='main'>
            ...
            {cart.length > 0 && <h3 className='mt-5'>Resumen</h3>}
            {cart.map(i => {
            ...
            {!isCartEmpty && <p style={{color: "red", margin: "auto 0", fontWeight: "bold"}}>Total: ${total}</p>}
            {!isCartEmpty && <button onClick={clear} style={{margin: 15}} className='btn btn-primary'>Vaciar carrito</button>}
            {isCartEmpty ? <button onClick={()=> navigate("../productos")} style={{margin: 15}} className='btn btn-primary'>Comenzar a comprar</button> 
                : <button onClick={()=> navigate("../productos")} style={{margin: 15}} className='btn btn-primary'>Seguir comprando</button>}


            {isCartEmpty && <p style={estilos}>En breve podrá ver su lista de compras en esta sección</p>}
        </main>
    )
}
